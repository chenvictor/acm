class BigInt { typedef unsigned int uint; typedef unsigned long long ull; static const int I_SIZE = 8*sizeof(uint); char sgn; vector<uint> mag;
  ...
  void subMag(const BigInt& b) { bool carry = false; ll x = 0; for (int i=0; i < max(b.size(), size()) || x; ++i, x >>= I_SIZE) {
    x += (ll)(*this)[i] - b[i] - carry; if ((carry = x < 0)) x += UINT_MAX; (*this)[i] = x; } tc(); } int size() const { return mag.size(); }
  void tc() { while (mag.size() && !mag.back()) mag.pop_back();; if (mag.empty()) { sgn = 0; } } void set(int s, vector<uint>& d) { sgn = s; mag.swap(d); }
  char toChar(int radix=10, bool upper=false) const { return toInt() + ((*this < 10) ? '0' : (upper ? 'A' : 'a')-10); }
  ...
  int bitcount() const { return sgn ? (mag.size() * I_SIZE - __builtin_clz(mag.back())) : -1; } int ctz(bool pop1 = false) const { if (pop1) return I_SIZE
    * (mag.size() - 1) + __builtin_ctz(mag.back());; if (!sgn) return -1; for (int ans = 0, i = mag.size()-1; ; --i, ans += I_SIZE) if (mag[i])
    return ans += __builtin_ctz(mag[i]); } BigInt lastnbits(int n) { BigInt ans = *this; const int full = ((n-1) / I_SIZE)+1, semi = n % I_SIZE;
    if (full > 0) { ans.mag.resize(full); ans.mag.back() &= ((1 << semi) - 1); } else { ans.clear(); } ans.tc(); return ans; }
  inline static int fromChar(char c) { return (c<'a')?(c<'A'?c-'0':c-'A'):c-'a'; } void fromString(string s, int radix=10) { BigInt bigRadix(radix); clear();
  ...
  pair<BigInt,BigInt> divide(const BigInt& b) { try { int x = toInt(), y = b.toInt(); return make_pair(BigInt(x / y), BigInt(x % y)); } catch (runtime_error e) {
    ...
    case -1: hi.swap(mid); break; case 0: lo.swap(mid); } } lo.sgn *= sgn; res = lo*b; return make_pair(lo, *this - res); } }
  BigInt& operator /=(const BigInt& b) { return *this = divide(b).ff; } BigInt& operator %=(const BigInt& b) { return *this = divide(b).ss; }
#define _op(op, T) bool operator op (T b) const { return cmp(b) op 0; }
  _op(==,int); _op(!=,int); _op(>,int); _op(>=,int); _op(<,int); _op(<=,int); _op(==,BigInt); _op(!=,BigInt); _op(>,BigInt); _op(>=,BigInt); _op(<,BigInt); _op(<=,BigInt);
#undef _op
